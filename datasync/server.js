...
const root = loader(path.join(__dirname, './routers/api'), '/nsop/datasync/api');
app.use(root.routes()).use(root.allowedMethods());          // 加载路由

app.listen(config.server.datasync.port);                    // 启动http服务

log.info({                                                  // 记录系统启动日志
    path: '~',
    type: 'start',
    port: config.server.datasync.port
}, 'dataasync v1.0 app services is starting at port ' + config.server.datasync.port);
